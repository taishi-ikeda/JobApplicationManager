...
namespace application_gui {
    ...
    private:
        ...
        void updateTableWidget() {
          auto& database = database::get();
            tableWidget_->setRowCount(0); // テーブルをクリア
      ...
      void addApplicationData(const database::application& app) {
        ...
        connect(deleteBtn, &QPushButton::clicked, [this, row]() {
            ...
            updateDataBase(); // テーブルの変更をデータベースに反映
        });
        ...
        connect(editBtn, &QPushButton::clicked, [app,this]() {
            ...
            // ここでdialogにappのデータを渡して初期化する処理を実装
            // 例: dialog.setApplicationData(app);
            if (dialog.exec() == QDialog::Accepted) {
                // フォームで編集されたデータをテーブルとデータベースに反映する処理を実装
                this->updateTableWidget(); // データベースの内容が変更されたと想定して、テーブルを更新
            }
        });
        ...
        if (method.contains("郵送")) {
          itemMethod->setForeground(QColor("#e67e22")); // 郵送は注意を促すためオレンジ色に
        }
        itemMethod->setFlags(itemMethod->flags() & ~Qt::ItemIsEditable); // 直接編集不可にする
        ...
        QStringList docs;
        const std::vector<database::application::item>& item_list 
        = app.get_item_list();
        ...
        for (const QString &doc : docs) {
            ...
            // チェックボックスの状態が変わったら、進捗率を再計算する
            connect(cb, &QCheckBox::checkStateChanged, [row,this]() { this->updateProgress(row); });
        }
        ...
        itemProgress->setFlags(itemProgress->flags() & ~Qt::ItemIsEditable); // 直接編集不可にする
      ...
      // 各行のチェックボックスの状態を見て、進捗率を更新する関数
      void updateProgress(int row) {
          auto *scrollWidget = tableWidget_->cellWidget(row, 4);
          ...
          // 進捗率セルの更新
          QTableWidgetItem *item = tableWidget_->item(row, 4);
          if (item) {
      ...
      void onSaveButtonClicked() {
        ...
        if(!filename.isEmpty()) {
            if(filename.endsWith(".json", Qt::CaseInsensitive) == false) {
              filename += ".json"; // 拡張子がない場合は.jsonを追加
            }
            if(filename.endsWith(".json", Qt::CaseInsensitive) == false) {
      ...
      void updateDataBase() {
        // テーブルの内容をデータベースに反映させる処理を実装
        auto& database = database::get();
        database.initialize(); // 既存のデータをクリア
        ...
        for (int row = 0; row < rowCount; ++row) {
            ...
            // フォルダパスとURLはテーブルには表示していないため、ここでは仮の値を入れる
            QUrl url;
            QDir dir;
      ...
      void openFormDialog() {
        // 新規公募の入力フォームを表示する処理を実装
         formDialog dialog(this);
        if (dialog.exec() == QDialog::Accepted) {
            // フォームで入力されたデータをテーブルに追加する処理を実装
            // ここでは仮にフォームから直接データベースに追加されると想定しているため、テーブルの更新だけ行う
            updateTableWidget();
        }
      }
